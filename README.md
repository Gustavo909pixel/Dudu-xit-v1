...
local Settings = {
    ...
    ESPHealthColor = Color3.fromRGB(255, 255, 255),
    HitboxSize = Vector3.new(10, 10, 10),
    GokuSpeed = 1.8, -- Velocidade da câmera livre
    RotateSpeed = 2  -- Velocidade de rotação
}

local espAtivo, mostrarVida, isGoku, hitboxAtiva, puloInfinito = false, false, false, false, false
_G.GokuAction = nil -- Variável global para os controles
...
local ostroke = Instance.new("UIStroke", open); ostroke.Color = Color3.fromRGB(255,255,255); ostroke.Thickness = 2

local frame = Instance.new("Frame", gui)
frame.Size = UDim2.new(0,320,0,380); frame.Position = UDim2.new(0.5,-160,0.3,0)
frame.BackgroundColor3 = Settings.MenuColor; frame.Visible = false; frame.Active = true
Instance.new("UICorner", frame)
local stroke = Instance.new("UIStroke", frame); stroke.Thickness = 2; stroke.Color = Color3.fromRGB(255, 255, 255)

local title = Instance.new("TextLabel", frame)
title.Text = "DUDU XIT V1"; title.Size = UDim2.new(1, 0, 0, 30); title.Position = UDim2.new(0, 0, 0, 5)
title.BackgroundTransparency = 1; title.Font = Enum.Font.GothamBold; title.TextSize = 18; title.TextColor3 = Color3.fromRGB(255, 255, 255)

local close = Instance.new("TextButton", frame)
close.Size = UDim2.new(0,30,0,30); close.Position = UDim2.new(1,-35,0,5); close.Text = "X"
close.BackgroundColor3 = Color3.fromRGB(255, 255, 255); close.TextColor3 = Color3.fromRGB(0, 0, 0)
...
local function createTab(name, pos)
    ...
    btn.Text = name; btn.BackgroundColor3 = Color3.fromRGB(30, 30, 30); btn.TextColor3 = Color3.fromRGB(255, 255, 255)
    btn.TextSize = 12; Instance.new("UICorner", btn)
    local tstroke = Instance.new("UIStroke", btn); tstroke.Color = Color3.fromRGB(255,255,255)
...
local function createBtn(parent, text)
    ...
    Instance.new("UICorner", b); local bstroke = Instance.new("UIStroke", b); bstroke.Color = Color3.fromRGB(255,255,255)
    return b
end

-- BOTÕES DAS ABAS
local espBtn = createBtn(espPage, "ESP: OFF")
local hpBtn = createBtn(espPage, "VIDA: OFF")
local hitboxBtn = createBtn(mainPage, "HITBOX: OFF")
local gokuBtn = createBtn(mainPage, "GOKU MODE: OFF")
local jumpBtn = createBtn(mainPage, "PULO INFINITO: OFF")

local rgbHitboxBtn = createBtn(configPage, "RGB HITBOX: ON")
local rgbTextBtn = createBtn(configPage, "RGB TEXTO: ON")
local rgbESPBtn = createBtn(configPage, "RGB ESP: ON")

-- ============ CONTROLES GOKU (CÂMERA LIVRE) ============
local gokuControls = Instance.new("Frame", gui)
gokuControls.Size = UDim2.new(0, 250, 0, 150); gokuControls.Position = UDim2.new(0.5, -125, 0.7, 0)
gokuControls.BackgroundTransparency = 1; gokuControls.Visible = false

local function createMoveBtn(name, pos, moveType)
    local b = Instance.new("TextButton", gokuControls)
    b.Size = UDim2.new(0, 45, 0, 45); b.Position = pos; b.Text = name
    b.BackgroundColor3 = Color3.fromRGB(0,0,0); b.TextColor3 = Color3.new(1,1,1)
    Instance.new("UICorner", b); local s = Instance.new("UIStroke", b); s.Color = Color3.new(1,1,1)
    
    b.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            _G.GokuAction = moveType
        end
    end)
    b.InputEnded:Connect(function() _G.GokuAction = nil end)
end

-- Botões de Movimento
createMoveBtn("▲", UDim2.new(0.4,0,0,0), "FWD")
createMoveBtn("▼", UDim2.new(0.4,0,0.6,0), "BWD")
createMoveBtn("◄", UDim2.new(0.2,0,0.3,0), "LFT")
createMoveBtn("►", UDim2.new(0.6,0,0.3,0), "RGT")
createMoveBtn("↑", UDim2.new(0.85,0,0,0), "UP")
createMoveBtn("↓", UDim2.new(0.85,0,0.6,0), "DN")
-- Botões de Rotação
createMoveBtn("R◄", UDim2.new(-0.1,0,0.3,0), "ROT_L")
createMoveBtn("R►", UDim2.new(1.0,0,0.3,0), "ROT_R")
        ...
        if plr ~= player and plr.Character then
            ...
            if root then
                ...
                if hitboxAtiva and isEnemyTarget then
                    root.Size = Settings.HitboxSize; root.Transparency = 0.5
                ...
                if hp then 
                    ...
                    if hum then hp.Frame.BAR.Size = UDim2.new(math.clamp(hum.Health/hum.MaxHealth, 0, 1), 0, 1, 0) end
                end
            end
        end
    end

    -- CORREÇÃO CÂMERA LIVRE (GOKU MODE)
    if isGoku then
        local look = camera.CFrame.LookVector
        local right = camera.CFrame.RightVector
        local moveDir = Vector3.new(0,0,0)
        
        if _G.GokuAction == "FWD" then moveDir += look end
        if _G.GokuAction == "BWD" then moveDir -= look end
        if _G.GokuAction == "RGT" then moveDir += right end
        if _G.GokuAction == "LFT" then moveDir -= right end
        if _G.GokuAction == "UP" then moveDir += Vector3.new(0,1,0) end
        if _G.GokuAction == "DN" then moveDir -= Vector3.new(0,1,0) end
        
        camera.CFrame = camera.CFrame + (moveDir * Settings.GokuSpeed)

        if _G.GokuAction == "ROT_L" then camera.CFrame *= CFrame.Angles(0, math.rad(Settings.RotateSpeed), 0) end
        if _G.GokuAction == "ROT_R" then camera.CFrame *= CFrame.Angles(0, math.rad(-Settings.RotateSpeed), 0) end
    end
end)
...
task.spawn(function()
    while true do
        ...
        for _, v in pairs(gui:GetDescendants()) do
            if (v:IsA("TextButton") or v:IsA("TextLabel")) and v.Name ~= "XIT" then
...
hpBtn.MouseButton1Click:Connect(function() mostrarVida = not mostrarVida; hpBtn.Text = "VIDA: "..(mostrarVida and "ON" or "OFF") end)
hitboxBtn.MouseButton1Click:Connect(function() hitboxAtiva = not hitboxAtiva; hitboxBtn.Text = "HITBOX: "..(hitboxAtiva and "ON" or "OFF") end)
jumpBtn.MouseButton1Click:Connect(function() puloInfinito = not puloInfinito; jumpBtn.Text = "PULO INFINITO: "..(puloInfinito and "ON" or "OFF") end)

gokuBtn.MouseButton1Click:Connect(function()
    isGoku = not isGoku
    gokuBtn.Text = "GOKU MODE: "..(isGoku and "ON" or "OFF")
    gokuControls.Visible = isGoku
    ...
    if isGoku then
        ...
        if player.Character then 
            ...
            player.Character.HumanoidRootPart.CFrame = CFrame.new(camera.CFrame.Position)
        end
    end
end)

rgbHitboxBtn.MouseButton1Click:Connect(function() Settings.HitboxRGB = not Settings.HitboxRGB; rgbHitboxBtn.Text = "RGB HITBOX: "..(Settings.HitboxRGB and "ON" or "OFF") end)
rgbTextBtn.MouseButton1Click:Connect(function() Settings.TextRGB = not Settings.TextRGB; rgbTextBtn.Text = "RGB TEXTO: "..(Settings.TextRGB and "ON" or "OFF") end)
rgbESPBtn.MouseButton1Click:Connect(function() Settings.ESPRGB = not Settings.ESPRGB; rgbESPBtn.Text = "RGB ESP: "..(Settings.ESPRGB and "ON" or "OFF") end)
